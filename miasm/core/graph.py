...
"""
        ...
        for node in self.nodes():
            ...
            for lineDesc in self.node2lines(node):
                ...
                for col in lineDesc:
                    out_render += "%s\n" % (
                        str(col.text))
                node_html_lines += out_render

            out += node_tmp % (self.nodeid(node), node_html_lines)
        ...
        for src, dst in self.edges():
            ...
            edges += 'edge: { sourcename: "%s" targetname: "%s" }\n' % (self.nodeid(src), self.nodeid(dst))


        return t + out + edges + end
